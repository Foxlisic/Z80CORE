...
void Z80Spectrum::cls(int cl) {
    ...
    if (sdl_enable && sdl_screen) {
        for (int _i = 0; _i < scale*scale*width*height; _i++)
...
#ifndef NO_SDL
    if (sdl_enable && sdl_screen) {
        ( (Uint32*)sdl_screen->pixels )[x + scale*width*y] = color;
    }
#endif
}
    ...
    if (x >= 0 && y >= 0 && x < width && y < height) {
        ...
        if (sdl_enable && sdl_screen) {
            for (int k = 0; k < scale*scale; k++)
                ( (Uint32*)sdl_screen->pixels )[ scale*(x + scale*width*y) + (k%scale) + scale*width*(k/scale) ] = get_color(color);
        }
#endif
