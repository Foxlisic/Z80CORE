...
void Z80Spectrum::main() {
    ...
    if (sdl_enable) {
        ...
        sdl_screen = SDL_SetVideoMode(scale*width, scale*height, 32, SDL_HWSURFACE | SDL_DOUBLEBUF);
    ...
    for (int u = 1; u < argc; u++) {
        ...
        if (argv[u][0] == '-') {
            ...
            switch (argv[u][1]) {
                ...
                // Показать дебаггер и останавливать на HALT
                case 'd': ds_show_dbg = 1; break;
